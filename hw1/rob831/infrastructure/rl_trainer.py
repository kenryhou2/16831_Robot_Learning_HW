...
class RL_Trainer(object):
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            train_log = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch) #DONE
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for path in paths:
            ...
            #get_action returns (T, ac_dim)

        return paths #Each path contains (T, ac_dim)
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        if self.log_metrics:
            ...
            # Calculate performance ratio (agent performance vs expert performance)
            
            expert_performance = np.mean(train_returns)
            agent_performance = np.mean(eval_returns)
            if expert_performance > 0:
                performance_ratio = agent_performance / expert_performance
            else:
                performance_ratio = 0.0
            logs["Performance_Percentage"] = performance_ratio * 100.0
